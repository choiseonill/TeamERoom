...
<script type="text/javascript">
	$().ready(function() {
			...
			if (value != "") {
				...
				$.post("<c:url value="/api/exists/email" />", {
					...
					if (response.response) {
						$("#RinputEmail").removeClass("valid");
						$("#RinputEmail").addClass("invalid");
						$("#EmailWarning").css("display", "inline-block");
					}

					else {
						$("#RinputEmail").removeClass("invalid");
						$("#RinputEmail").addClass("valid");
		...
		$("#RinputPassword").keyup(function() {
			...
			var password = $("#RinputPassword-confirm").val();

			if (value != password) {
				$(this).removeClass("valid");
				$(this).addClass("invalid");
				$("#RinputPassword-confirm").removeClass("valid");
				$("#RinputPassword-confirm").addClass("invalid");
				$("#PwWarning").css("display", "inline-block");
			} else {
				$(this).removeClass("invalid");
				$(this).addClass("valid");
				$("#RinputPassword-confirm").removeClass("invalid");
				$("#RinputPassword-confirm").addClass("valid");
				$("#PwWarning").css("display", "none");
			}
		...
		$("#RinputPassword-confirm").keyup(function() {
			...
			if (value != password) {
				$(this).removeClass("valid");
				$(this).addClass("invalid");
				$("#RinputPassword").removeClass("valid");
				$("#RinputPassword").addClass("invalid");
				$("#PwWarning").css("display", "inline-block");
			} else {
				$(this).removeClass("invalid");
				$(this).addClass("valid");
				$("#RinputPassword").removeClass("invalid");
				$("#RinputPassword").addClass("valid");
				$("#PwWarning").css("display", "none");
			}
		})
		...
		<div class="card card-container" style="width: 400px;">
			<div>
				<div class="nbutton"
					...
					<label for="nomal">일반</label> <input type="radio" id="nomal"
						name="select" />
				</div>
				<div class="rbutton"
					style="display: inline-block; verticl-align: top; margin-left: 30%;">
					<label for="host">사업자</label> <input type="radio" id="host"
						name="select" />
				</div>
			</div>
			...
			<form:form class="form-signin" modelAttribute="registForm"
				...
				<input type="text" id="RinputEmail" class="form-control"
					name="email" placeholder="Email address" required autofocus />
				<span id="EmailWarning"> 사용 불가능한 이메일 입니다. </span>
				<input type="password" id="RinputPassword" class="form-control"
					name="password" placeholder="Password" required />
				<input type="password" id="RinputPassword-confirm"
					class="form-control" name="password-confirm"
					placeholder="Password-Confirm" required />
				<span id="PwWarning"> 비밀번호와 비밀번호확인이 일치 하지않습니다. </span>	
				<hr>
				...
				<select id="question" class="form-control" name="question">
					<option value="1">아이디/비번 찾기 질문</option>
				...
				<input type="text" id="BizName" class="form-control" name="bizName"
					placeholder="사업장 명" />
				<input type="text" id="BizNumber" class="form-control"
					name="bizNumber" placeholder="사업자 번호 " />
				<hr>
				<input type="file" id="profilePhoto" class="form-control"
					title="프로필 사진" name="file" />
